---
title: Deploy deepnova using aaPanel
description: >-
  Learn how to deploy the deepnova service using aaPanel-Docker, including installing the Docker container environment and using the command to start the service with one click. Detailed instructions on how to configure environment variables and use proxy addresses.
...
3. After the installation is complete, find `deepnova` in `One-Click Install` and click `install`\
   ![install-deepnova](https://github.com/user-attachments/assets/9f989104-bb8e-4acd-9721-6b1db1017d2b)

4. configure basic information such as the domain name, OpenAI API key, and port to complete the installation Note: The domain name is optional, if the domain name is filled, it can be managed through \[Website]--> \[Proxy Project], and you do not need to check \[Allow external access] after filling in the domain name, otherwise you need to check it before you can access it through the port ![adddeepnova](https://github.com/user-attachments/assets/f0b2e72d-9eee-46a8-b094-4834b78764df)

5. After installation, enter the domain name or IP+ port set in the previous step in the browser to access.

- Name: application name, default `deepnova-random characters`
...
- If you have set a domain name, please directly enter the domain name in the browser address bar, such as `http://demo.deepnova`, to access the `deepnova` console.
